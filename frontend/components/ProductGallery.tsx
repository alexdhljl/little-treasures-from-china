...
export function ProductGallery({ images, alt }: { images: string[]; alt: string }) {
  ...
  if (!count) return <div className="grid aspect-[4/5] place-items-center bg-[#f2f0eb] text-sm text-[#777]">No image available</div>;
  ...
  return <>
    <div className="relative overflow-hidden bg-[#f2f0eb]" onTouchStart={(event) => { touchStart.current = event.touches[0].clientX; }} onTouchEnd={(event) => swipeEnd(event.changedTouches[0].clientX)}>
      <button aria-label="Open fullscreen image" className="grid aspect-[4/5] max-h-[720px] w-full place-items-center" onClick={() => setLightbox(true)} type="button">
        <img alt={`${alt} ${index + 1}`} className="h-full w-full object-cover" src={image} />
      </button>
      {count > 1 ? <>
    ...
    {count > 1 ? <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{cleanImages.map((src, imageIndex) => <button aria-label={`View image ${imageIndex + 1}`} className={`relative aspect-[4/5] w-[68px] shrink-0 overflow-hidden border-2 bg-[#f2f0eb] ${imageIndex === index ? "border-[#171717]" : "border-transparent"}`} key={`${src}-${imageIndex}`} onClick={() => setIndex(imageIndex)} type="button"><img alt="" className="h-full w-full object-cover" loading="lazy" src={src} /></button>)}</div> : null}
