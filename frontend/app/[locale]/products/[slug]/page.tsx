  ...
  return <main className="min-h-screen bg-white text-[#171717]">
    ...
    <section className="mx-auto grid max-w-7xl gap-7 px-4 pb-10 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:gap-12 lg:pb-16">
      ...
      <aside className="pt-1 lg:pt-3">
        ...
        <dl className="mt-6 border-t border-black/15 text-sm"><InfoRow label={locale === "zh" ? "博物馆" : "Museum"} value={displayName(product.museum, locale) || (locale === "zh" ? "精选合作机构" : "Curated partner")} /><InfoRow label={locale === "zh" ? "系列" : "Collection"} value={product.collection || product.officialCollection || (locale === "zh" ? "待确认" : "To be confirmed")} /><InfoRow label="MOQ" value={String(product.moq || 1)} /><InfoRow label={locale === "zh" ? "材质" : "Material"} value={product.materials || (locale === "zh" ? "待确认" : "To be confirmed")} /><InfoRow label={locale === "zh" ? "尺寸" : "Dimensions"} value={product.dimensions || (locale === "zh" ? "待确认" : "To be confirmed")} /><InfoRow label={locale === "zh" ? "产地" : "Origin"} value={product.origin || origin} /><InfoRow label={locale === "zh" ? "交付周期" : "Lead Time"} value={product.leadTime || product.shippingNote || (locale === "zh" ? "报价时确认" : "Confirmed with quote")} /><InfoRow label={locale === "zh" ? "库存" : "Availability"} value={inventoryLabel(product.inventoryStatus, locale)} /></dl>
        <div className="mt-6"><AddToInquiryButton locale={locale} product={{ id: product.id, slug: product.slug, name: title, image: product.images[0] || "" }} /></div>
