...
function parseCsv(content: string) {
  ...
  return values.filter(values => values.some(Boolean)).map(values => { if (values.length !== header.length) throw new Error("CSV record has wrong column count"); return Object.fromEntries(header.map((title, index) => [aliases[title], values[index] || null])); });
}
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ path?: string[] }> }) { try {
  ...
  if (path[0] === "import" && path[1] === "seeds") return NextResponse.json(await importRecords((seedRows as unknown as LeadRecord[]).map(seed)));
  if (path[0] === "import" && path[1] === "csv") return NextResponse.json(await importRecords(parseCsv(String(body.content || ""))));
  if (path[0] === "import" && path[1] === "records") return NextResponse.json(await importRecords(Array.isArray(body.records) ? body.records as LeadRecord[] : []));
  ...
  const names = [...new Set(people.map(normalizedText).filter(Boolean))]; const target = Number(body.target_per_person) || 100;
  ...
  return NextResponse.json({ assigned_now: assigned, counts, shortfall: Object.fromEntries(names.map(name => [name, Math.max(0,target-counts[name])])) });
