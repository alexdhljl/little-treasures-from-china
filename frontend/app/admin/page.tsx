  ...
  return (
    <form className="grid gap-6" onSubmit={(event) => { event.preventDefault(); onSave(); }}>
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="Product Name"><input className={inputClass} required value={value.englishName} onChange={(event) => onChange({ ...value, englishName: event.target.value, slug: value.slug || slugify(event.target.value) })} /></Field>
        <Field label="Chinese / Local Name"><input className={inputClass} value={value.name} onChange={(event) => set("name", event.target.value)} /></Field>
        ...
        <Field label="Status"><select className={inputClass} value={value.status} onChange={(event) => set("status", event.target.value as ProductInput["status"])}><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></select></Field>
        ...
        <Field label="Estimated Price"><input className={inputClass} min="0" step="0.01" type="number" value={value.price ?? ""} onChange={(event) => set("price", event.target.value ? Number(event.target.value) : null)} /></Field>
      ...
      <Field label="Description"><textarea className={inputClass} rows={3} value={value.shortDescription} onChange={(event) => set("shortDescription", event.target.value)} /></Field>
      <Field label="Story"><textarea className={inputClass} rows={8} value={value.story} onChange={(event) => set("story", event.target.value)} /></Field>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="SEO Title"><input className={inputClass} value={value.seoTitle} onChange={(event) => set("seoTitle", event.target.value)} /></Field>
        <Field label="SEO Description"><textarea className={inputClass} rows={3} value={value.seoDescription} onChange={(event) => set("seoDescription", event.target.value)} /></Field>
        <Field label="Image Alt Text"><input className={inputClass} value={value.altText} onChange={(event) => set("altText", event.target.value)} /></Field>
      ...
      <div className="grid gap-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {value.images.map((image, index) => <div className="border border-black/10 bg-white p-2" key={image}><div className="relative"><img alt={value.altText} className="aspect-[4/3] w-full object-cover" loading="lazy" src={image} />{index === 0 ? <span className="absolute left-2 top-2 bg-[#171717] px-2 py-1 text-[10px] font-black uppercase text-white">Cover</span> : null}</div><div className="mt-2 flex items-center justify-between"><GripVertical size={16} /><div className="flex gap-1"><button aria-label="Move left" className="border p-1.5" type="button" onClick={() => moveImage(index, index - 1)}><ArrowLeft size={14} /></button><button aria-label="Move right" className="border p-1.5" type="button" onClick={() => moveImage(index, index + 1)}><ArrowRight size={14} /></button><button aria-label="Delete image" className="border p-1.5 text-red-700" type="button" onClick={() => set("images", value.images.filter((_, imageIndex) => imageIndex !== index))}><Trash2 size={14} /></button></div></div></div>)}
        </div>
      </div>
...
const cmsFields: Record<"categories" | "museums" | "collections" | "stories", Array<{ key: string; label: string; type?: string }>> = {
  ...
  museums: [{ key: "name", label: "Museum Name" }, { key: "nameZh", label: "Chinese Name" }, { key: "slug", label: "Slug" }, { key: "province", label: "Province" }, { key: "city", label: "City" }, { key: "logo", label: "Logo URL" }, { key: "coverImage", label: "Cover URL" }, { key: "description", label: "Description", type: "textarea" }, { key: "story", label: "Story", type: "textarea" }, { key: "sortOrder", label: "Sort Order", type: "number" }, { key: "featured", label: "Featured", type: "checkbox" }],
  collections: [{ key: "name", label: "Collection Name" }, { key: "nameZh", label: "Chinese Name" }, { key: "slug", label: "Slug" }, { key: "museumId", label: "Museum", type: "museum" }, { key: "bannerImage", label: "Banner URL" }, { key: "description", label: "Description", type: "textarea" }, { key: "story", label: "Story", type: "textarea" }, { key: "sortOrder", label: "Sort Order", type: "number" }, { key: "featured", label: "Featured", type: "checkbox" }],
...
export default function AdminPage() {
  ...
  async function loadAll(token = accessToken) {
    ...
    try {
      const result = await Promise.all([fetchProducts(token), fetchCategories(token), fetchMuseums(token), fetchCollections(token), fetchStories(token), fetchMedia(token), fetchSiteSettings(token)]);
      setProducts(result[0]); setCategories(result[1]); setMuseums(result[2]); setCollections(result[3]); setStories(result[4]); setMedia(result[5]); setSettings(result[6]); setStatus("");
